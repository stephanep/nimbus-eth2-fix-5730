    ...
    let
      keystore =
        createKeystore(
          ...
          description = "This is a test keystore that uses scrypt to secure the secret.",
          path =
            validateKeyPath("m/12381/60/3141592653/589793238").expect("Valid keypath"),
        )
    var
      encryptJson = parseJson Json.encode(keystore)
...
suite "eth2.0-deposits-cli compatibility":
  test "restoring mnemonic without password":
    let
      mnemonic =
        Mnemonic "camera dad smile sail injury warfare grid kiwi report minute fold slot before stem firm wet vague shove version medal one alley vibrant mushroom"
  ...
  test "restoring mnemonic with password":
    let
      mnemonic =
        Mnemonic "swear umbrella lesson couch void gentle rocket valley distance match floor rocket flag solve muscle common modify target city youth pottery predict flip ghost"
