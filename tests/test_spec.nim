...
suite "Beacon state" & preset():
  ...
  test "latest_block_root":
    ...
    let
      blck =
        addTestBlock(state[], cache, nextSlot = false, flags = {skipBlsValidation}).phase0Data
  ...
  test "get_beacon_proposer_index":
    ...
    check:
      ...
      get_beacon_proposer_index(state[].phase0Data.data, cache, Epoch(1).start_slot()).isNone()
    ...
    check:
      ...
      get_beacon_proposer_index(state[].phase0Data.data, cache, Epoch(1).start_slot()).isSome()
