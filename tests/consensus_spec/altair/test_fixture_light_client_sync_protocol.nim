...
proc get_sync_aggregate(
  ...
  let
    ...
    committee_indices =
      signature_state.current_sync_committee.pubkeys.mapIt(
  ...
  suite "EF - " & $storeDataFork & " - Unittests - Light client - Sync protocol" &
    ...
    test "test_process_light_client_update_not_timeout":
      ...
      let
        ...
        update =
          storeDataFork.LightClientUpdate(
    ...
    test "test_process_light_client_update_at_period_boundary":
      ...
      process_slots(cfg, forked[], Slot(UPDATE_TIMEOUT - 2), cache, info, flags = {}).expect(
        "no failure"
      )
      let
    ...
    test "process_light_client_update_finality_updated":
      ...
      process_slots(cfg, forked[], Slot(SLOTS_PER_EPOCH * 2), cache, info, flags = {}).expect(
        "no failure"
      )
