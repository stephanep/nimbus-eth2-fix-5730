...
proc runTest(
  ...
  test testName & " - " & unitTestName & preset():
    let
      preState = newClone(parseTest(testPath / "pre.ssz_snappy", SSZ, AnteBeaconState))
    var
...
suite "EF - Altair - Transition " & preset():
  ...
  for kind, path in walkDir(TransitionDir, relative = true, checkDir = true):
    ...
    runTest(
      ...
      cfg,
      "EF - Altair - Transition",
      TransitionDir,
      suiteName,
      path,
      transitionInfo.fork_block,
    )
