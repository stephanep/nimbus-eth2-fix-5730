...
suite "Gossip validation " & preset():
  ...
  test "validateAttestation":
    ...
    for blck in makeTestBlocks(
      ...
      let
        added =
          dag.addHeadBlock(verifier, blck.phase0Data) do(
            blckRef: BlockRef,
    ...
    check:
      ...
      validateAttestation(pool, batchCrypto, att_1_0, beaconTime, subnet, true).waitFor().error()[
        0
    ...
    block:
      ...
      check:
        ...
        validateAttestation(pool, batchCrypto, broken, beaconTime, subnet, true).waitFor().error()[
          0
...
suite "Gossip validation - Altair":
  ...
  proc getFirstAggregator(
    ...
    for i, index in dag.syncCommitteeParticipants(signatureSlot):
      ...
      let
        ...
        proofFut =
          validator.getSyncCommitteeSelectionProof(
  ...
  proc getSyncCommitteeMessage(
    ...
    let
      ...
      msgFut =
        validator.getSyncCommitteeMessage(
          getStateField(dag.headState, fork),
          getStateField(dag.headState, genesis_validators_root),
          msgSlot,
  ...
  test "Period boundary":
    ...
    proc checkWithSignatureSlot(signatureSlot: Slot, expectValid: bool) =
      ...
      let
        ...
        msgVerdict =
          waitFor dag.validateSyncCommitteeMessage(
            quarantine,
            batchCrypto,
            syncCommitteePool,
            msg,
        ...
        (bid, cookedSig, positions) =
          if msgVerdict.isOk:
            ...
            let
              ...
              positionsInSubcommittee =
                dag.getSubcommitteePositions(
      ...
      let
        contrib =
          block:
            let
              contrib =
                (ref SignedContributionAndProof)(
                  message:
            ...
            let
              signRes =
                waitFor validator.getContributionAndProofSignature(
                  getStateField(dag.headState, fork),
                  getStateField(dag.headState, genesis_validators_root),
                  contrib[].message,
                )
      ...
      let
        contribVerdict =
          waitFor dag.validateContribution(
            quarantine,
            batchCrypto,
            syncCommitteePool,
            contrib[],
  ...
  test "validateSyncCommitteeMessage - Duplicate pubkey":
    ...
    let
      ...
      res =
        waitFor validateSyncCommitteeMessage(
          dag,
          quarantine,
          batchCrypto,
          syncCommitteePool,
