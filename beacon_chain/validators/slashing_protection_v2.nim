...
proc checkDB(
  ...
  let
    status =
      selectStmt.exec do(res: (int64, Hash32)):
        version = res[0]
        root.data = res[1]
...
proc getMetadataTable_DbV2*(db: SlashingProtectionDB_v2): Opt[Eth2Digest] =
  ...
  let
    v2exists =
      existenceStmt.exec do(res: int64):
        hasV2 = res
...
proc getValidatorInternalID(
  ...
  let
    status =
      db.sqlGetValidatorInternalID.exec(serializedPubkey) do(res: ValidatorInternalID):
        valID = res
...
proc checkSlashableBlockProposalOther(
  ...
  block:
    ...
    let
      status =
        db.sqlBlockMinSlot.exec(valID) do(res: int64):
          minSlot = res
    if status.foundAnyResult():
...
proc checkSlashableBlockProposalDoubleProposal(
  ...
  block:
    ...
    let
      status =
        db.sqlBlockForSameSlot.exec((valID, int64 slot)) do(res: Hash32):
          root.data = res
...
proc checkSlashableAttestationDoubleVote(
  ...
  block:
    ...
    let
      status =
        db.sqlAttForSameTargetEpoch.exec((valID, int64 target)) do(res: Hash32):
          root.data = res
...
proc checkSlashableAttestationOther(
  ...
  block:
    ...
    let
      status =
        db.sqlAttMinSourceTargetEpochs.exec(valID) do(res: tuple[source, target: int64]):
...
proc pruneAttestations*(
  ...
  let
    status =
      db.sqlPruneValidatorAttestations.exec(
...
proc pruneAfterFinalization*(db: SlashingProtectionDB_v2, finalizedEpoch: Epoch) =
  ...
  block: # Prune attestations
    let
      status =
        db.sqlPruneAfterFinalizationAttestations.exec(
...
proc retrieveLatestValidatorData*(
  ...
  let
    status =
      db.sqlMaxBlockAtt.exec(valID) do(res: tuple[slot, source, target: int64]):
        slot = res.slot
        source = res.source
        target = res.target
...
proc registerSyntheticAttestation*(
  ...
  block:
    let
      status = db.sqlInsertAtt.exec((valID, int64 source, int64 target, ZERO_HASH.data))
    checkStatus()
  block:
...
proc toSPDIR*(db: SlashingProtectionDB_v2): SPDIR {.raises: [IOError].} =
  ...
  block:
    ...
    let
      status =
        selectRootStmt.exec do(res: Hash32):
  ...
  block:
    ...
    let
      status =
        selectValStmt.exec do(res: PubKeyBytes):
  ...
  block:
    ...
    for i in 0..<result.data.len:
      ...
      block: # Blocks
        let
          status =
            selectBlkStmt.exec(validator.pubkey.PubKeyBytes) do(
      ...
      block: # Attestations
        let
          status =
            selectAttStmt.exec(validator.pubkey.PubKeyBytes) do(
