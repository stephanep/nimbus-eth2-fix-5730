    ...
    let
      signature =
        block:
          let
            res =
              await registered.validator.getAttestationSignature(
...
proc getBlockProposalEth1Data*(
  ...
  result =
    node.elManager.getBlockProposalData(
...
proc getBuilderBid[
  ...
  let
    blindedBlockParts =
      await getBlindedBlockParts[EPH](
        node,
  ...
  let
    unsignedBlindedBlock =
      getUnsignedBlindedBeaconBlock[SBBB](
  ...
  withBlck(forkedBlck):
    when consensusFork >= ConsensusFork.Capella:
      when (
        ...
        return err(
            "makeBlindedBeaconBlockForHeadAndSlot: mismatched block/payload types"
          )
    else:
...
proc proposeBlock(
  ...
  template proposeBlockContinuation(type1, type2: untyped): auto =
    await proposeBlockAux(
      ...
      node.config.localBlockValueBoost,
    )
...
proc handleAttestations(node: BeaconNode, head: BlockRef, slot: Slot) =
  ...
  let
    ...
    genesis_validators_root = node.dag.genesis_validators_root
    registeredRes =
      node.attachedValidators.slashingProtection.withContext:
        var tmp: seq[(RegisteredAttestation, SubnetId)]

        for committee_index in get_committee_indices(committees_per_slot):
          let
            committee =
              get_beacon_committee(epochRef.shufflingRef, slot, committee_index)
            subnet_id =
...
proc createAndSendSyncCommitteeMessage(
  ...
  try:
    let
      ...
      msg =
        block:
          let
            res =
              await validator.getSyncCommitteeMessage(
    ...
    let
      res =
        await node.router.routeSyncCommitteeMessage(
    ...
    let
      ...
      selectionProof =
        block:
          let
            res =
              await validator.getSyncCommitteeSelectionProof(
  ...
  try:
    let
      ...
      selectionProof =
        block:
          let
            res = await validator.getSlotSignature(fork, genesis_validators_root, slot)
          if res.isErr():
    ...
    msg.signature =
      block:
        let
          res =
            await validator.getAggregateAndProofSignature(
              fork, genesis_validators_root, msg.message
            )
  ...
  try:
    ...
    for key in attachedValidatorPubkeys:
      ...
      if validator.externalBuilderRegistration.isSome:
        ...
        let
          validatorRegistration = await node.getValidatorRegistration(validator, epoch)
...
proc handleValidatorDuties*(node: BeaconNode, lastSlot, slot: Slot) {.async.} =
  ...
  let
    handleSyncCommitteeContributionsFut =
