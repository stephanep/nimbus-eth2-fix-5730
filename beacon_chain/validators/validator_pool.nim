...
proc getEpochSignature*(
  ...
  let
    signature =
      ...
      of ValidatorKind.Local:
        SignatureResult.ok(
          get_epoch_signature(fork, genesis_validators_root, epoch, v.data.privateKey).toValidatorSig()
        )
      of ValidatorKind.Remote:
...
proc getSlotSignature*(
  ...
  let
    signature =
      ...
      of ValidatorKind.Local:
        SignatureResult.ok(
          get_slot_signature(fork, genesis_validators_root, slot, v.data.privateKey).toValidatorSig()
        )
      of ValidatorKind.Remote:
...
proc getValidatorExitSignature*(
  ...
  return case v.kind
    ...
    of ValidatorKind.Remote:
      let
        request = Web3SignerRequest.init(fork, genesis_validators_root, voluntary_exit)
      await v.signData(request)
...
proc getDepositMessageSignature*(
  ...
  return case v.kind
    of ValidatorKind.Local:
      SignatureResult.ok(
        get_deposit_signature(deposit_message, version, v.data.privateKey).toValidatorSig()
      )
    of ValidatorKind.Remote:
...
proc getBuilderSignature*(
  ...
  return case v.kind
    of ValidatorKind.Local:
      SignatureResult.ok(
        get_builder_signature(fork, validatorRegistration, v.data.privateKey).toValidatorSig()
      )
    of ValidatorKind.Remote:
