...
const
  ...
  mostCommonPasswords =
    wordListArray(
      nimbusSecurityResourcesPath / "passwords" /
...
proc loadLocalKeystoreImpl(
  ...
  let
    res =
      keyboardGetPassword[ValidatorPrivKey](
        prompt,
        3,
  ...
  let
    ...
    keys =
      try:
        ...
        let
          res = decodeBytes(Web3SignerKeysResponse, response.data, response.contentType)
        if res.isErr():
  ...
  try:
    ...
    for kind, file in walkDir(validatorsDir):
      if kind == pcDir:
        ...
        let
          keystore =
            loadKeystore(validatorsDir, secretsDir, keyName, nonInteractive, cache).valueOr:
...
proc loadNetKeystore*(
  ...
  if insecurePwd.isSome():
    ...
    let
      res =
        keyboardGetPassword[lcrypto.PrivateKey](
          prompt,
          3,
...
proc saveNetKeystore*(
  ...
  try:
    ...
    return err(
        KeystoreGenerationError(kind: FailedToCreateKeystoreFile, error: exc.msg)
      )
  ...
  let
    encodedStorage =
      try:
        ...
        return err(
            KeystoreGenerationError(kind: FailedToCreateKeystoreFile, error: e.msg)
          )
    ...
    res =
      keyboardGetPassword[KeySeed](
        ...
        proc(password: string): KsResult[KeySeed] =
            ...
            let
              status =
                decryptCryptoField(wallet.crypto, KeystorePass.init password, secret)
            case status
            of DecryptionStatus.Success:
              ok(KeySeed secret)
            else:
