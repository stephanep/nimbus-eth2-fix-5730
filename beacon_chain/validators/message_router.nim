      ...
      if blobsOpt.isSome:
        ...
        if blobs.len > 0 or kzgCommits.len > 0:
          let
            res =
              validate_blobs(
                kzgCommits, blobs.mapIt(it.blob), blobs.mapIt(it.kzg_proof)
              )
          if res.isErr():
...
proc routeAttestation*(
  ...
  block:
    let
      res =
        await router[].processor.processAttestation(
...
proc routeSignedAggregateAndProof*(
  ...
  block:
    ...
    let
      res =
        await router[].processor.processSignedAggregateAndProof(
...
proc routeSyncCommitteeMessage*(
  ...
  block:
    let
      res =
        await router[].processor.processSyncCommitteeMessage(
            ...
            block:
              ...
              for index, msg in msgs:
                if msg.validator_index < lenu64(forkyState.data.validators):
                  ...
                  statuses[index] =
                    Opt.some(SendResult.err("Incorrect validator's index"))
              ...
              for subcommitteeIdx in SyncSubcommitteeIndex:
                for valKey in syncSubcommittee(
                  ...
                  if index >= 0:
                    ...
                    resFutures.add(
                      router.routeSyncCommitteeMessage(
...
proc routeBlsToExecutionChange*(
  ...
  block:
    let
      res =
        await router.processor.processBlsToExecutionChange(
  ...
  let
    res = await router[].network.broadcastBlsToExecutionChange(bls_to_execution_change)
  if res.isOk():
