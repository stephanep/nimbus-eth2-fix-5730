...
proc installValidatorApiHandlers*(router: var RestRouter, node: BeaconNode) =
    ...
    let
      qepoch =
        block:
          if epoch.isErr():
            return RestApiResponse.jsonError(
                Http400, InvalidEpochValueError, $epoch.error()
              )
          let
            res = epoch.get()
          ...
          let
            qslot =
              block:
                if slot.isErr():
                  return RestApiResponse.jsonError(
                      Http400, InvalidSlotValueError, $slot.error()
                    )
                let res = slot.get()
          ...
          let
            qrandao =
              if randao_reveal.isNone():
                ...
                if res.isErr():
                  return RestApiResponse.jsonError(
                      Http400, InvalidRandaoRevealValue, $res.error()
                    )
                res.get()
          let
            qgraffiti =
  ...
  router.api(MethodGet, "/eth/v1/validator/blinded_blocks/{slot}") do(
    ...
    withConsensusFork(contextFork):
      when consensusFork >= ConsensusFork.Capella:
        let
          res =
            await makeBlindedBeaconBlockForHeadAndSlot[consensusFork.BlindedBeaconBlock](
    ...
    withConsensusFork(node.dag.cfg.consensusForkAtEpoch(qslot.epoch)):
      when consensusFork >= ConsensusFork.Capella:
        let
          ...
          headers =
            consensusFork.getMaybeBlindedHeaders(
          ...
          let
            qslot =
              block:
                ...
                if res.isErr():
                  return RestApiResponse.jsonError(
                      Http400, InvalidSlotValueError, $res.error()
                    )
                res.get()
            ...
            qindex =
              block:
                if committee_index.isNone():
                  return RestApiResponse.jsonError(
                      Http400, MissingCommitteeIndexValueError
                    )
    ...
    let
      attestation =
        block:
            ...
            qroot =
              block:
                if attestation_data_root.isNone():
                  return RestApiResponse.jsonError(
                      Http400, MissingAttestationDataRootValueError
                    )
          ...
          if res.isNone():
            return RestApiResponse.jsonError(
                Http400, UnableToGetAggregatedAttestationError
              )
          res.get()
  ...
  router.api(MethodPost, "/eth/v1/validator/beacon_committee_subscriptions") do(
    ...
    for request in requests:
      ...
      let
        validator_pubkey =
          getStateField(node.dag.headState, validators).item(request.validator_index).pubkey
    ...
    let
      proofs =
        block:
          ...
          if dres.isErr():
            return RestApiResponse.jsonError(
                Http400, InvalidContributionAndProofMessageError
              )
          dres.get()
      ...
      indexList =
        block:
          ...
          for item in items:
            ...
            if index in dupset:
              return RestApiResponse.jsonError(
                  Http400, DuplicateValidatorIndexArrayError
                )
            dupset.incl(index)
            res.add(index)
          if len(res) == 0:
