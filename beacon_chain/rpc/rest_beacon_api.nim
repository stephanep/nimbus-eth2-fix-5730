    ...
    for item in validatorIds:
      ...
      of ValidatorQueryKind.Key:
        ...
        if keyset.containsOrIncl(item.key):
          return err(
              RestErrorMessage.init(Http400, NonUniqueValidatorIdError, $item.key)
            )
          ...
          vindex =
            item.index.toValidatorIndex().valueOr:
              ...
              of ValidatorIndexError.TooHighValue:
                return err(
                    RestErrorMessage.init(Http400, TooHighValidatorIndexValueError)
                  )
              of ValidatorIndexError.UnsupportedValue:
                return err(
                    RestErrorMessage.init(Http500, UnsupportedValidatorIndexValueError)
                  )
          ...
          block:
            ...
            if len(indices) == 0:
              ...
              for index in indices:
                ...
                if status in validatorsMask:
                  res.add(
                    RestValidator.init(index, balance, toString(status), validator)
                  )
            res
      return RestApiResponse.jsonResponseFinalized(
    ...
    let
      vepoch =
        if epoch.isSome():
          ...
          if repoch.isErr():
            return RestApiResponse.jsonError(
                Http400, InvalidEpochValueError, $repoch.error()
              )
          let res = repoch.get()
    ...
    let
      vslot =
        if slot.isSome():
          ...
          if rslot.isErr():
            return RestApiResponse.jsonError(
                Http400, InvalidSlotValueError, $rslot.error()
              )
          let res = rslot.get()
          if vepoch.isSome():
                  ...
                  res =
                    if broadcast_validation.isNone():
                      ...
                      broadcast_validation.get().valueOr:
                        return RestApiResponse.jsonError(
                            Http400, InvalidBroadcastValidationType
                          )
  ...
  router.api(MethodPost, "/eth/v1/beacon/pool/attestations") do(
    ...
    if len(failures) > 0:
      return RestApiResponse.jsonErrorList(
          Http400, AttestationValidationError, failures
        )
    else:
    ...
    let
      messages =
        block:
          ...
          if dres.isErr():
            return RestApiResponse.jsonError(
                Http400, InvalidSyncCommitteeSignatureMessageError
              )
          dres.get()
