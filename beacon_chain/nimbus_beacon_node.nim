...
proc loadChainDag(
  ...
  proc onLightClientOptimisticUpdate(data: ForkedLightClientOptimisticUpdate) =
    ...
    withForkyOptimisticUpdate(data):
      when lcDataFork > LightClientDataFork.None:
        let
          contextFork = dag.cfg.consensusForkAtEpoch(forkyOptimisticUpdate.contextEpoch)
                  ...
                  try:
                    if metadata.genesis.kind == BakedInUrl:
                      info "Obtaining genesis state",
                        sourceUrl =
                          $config.genesisStateUrl.get(parseUri metadata.genesis.url)
...
func forkDigests(node: BeaconNode): auto =
  let
    forkDigestsArray: array[ConsensusFork, auto] = [
...
proc updateGossipStatus(node: BeaconNode, slot: Slot) {.async.} =
  ...
  const
    removeMessageHandlers: array[ConsensusFork, auto] = [
  ...
  const
    addMessageHandlers: array[ConsensusFork, auto] = [
