...
func block_signature_set*(
  ...
  let
    signing_root = compute_block_signing_root(fork, genesis_validators_root, slot, blck)
...
func aggregate_and_proof_signature_set*(
  ...
  let
    signing_root =
      compute_aggregate_and_proof_signing_root(
...
func sync_committee_message_signature_set*(
  ...
  let
    signing_root =
      compute_sync_committee_message_signing_root(
...
func sync_committee_selection_proof_set*(
  ...
  let
    signing_root =
      compute_sync_committee_selection_proof_signing_root(
      ...
      withState(state):
        when consensusFork >= ConsensusFork.Capella:
          for bls_change in signed_block.message.body.bls_to_execution_changes:
            let
              sig =
                bls_change.signature.load.valueOr:
                  return err(
            ...
            let
              validator_pubkey =
                bls_change.message.from_bls_pubkey.loadWithCache.valueOr:
                  return err(
                      "collectSignatureSets: cannot load BLS to execution change pubkey"
                    )
