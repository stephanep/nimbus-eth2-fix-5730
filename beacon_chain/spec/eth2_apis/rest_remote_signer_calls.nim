        ...
        message =
          block:
            let
              res =
                decodeBytes(
                  Web3SignerErrorResponse, response.data, response.contentType
                )
            if res.isErr():
  ...
  while true:
    ...
    if not (operationFut.finished()):
      ...
      if lastError.isSome():
        ...
        return Web3SignerDataResponse.err(
            Web3SignerError.init(
              Web3SignerErrorKind.TimeoutError, "Operation timed out"
            )
          )
    else:
