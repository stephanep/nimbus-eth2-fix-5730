...
proc serveAggregateAndProof*(
  ...
  let
    signature =
      try:
        let
          res = await validator.getAggregateAndProofSignature(fork, genesisRoot, proof)
        if res.isErr():
...
proc produceAndPublishAttestations*(
  ...
  let
    data = await vc.produceAttestationData(slot, committee_index, ApiStrategyKind.Best)
      ...
      statistics =
        block:
          ...
          try:
            ...
            let
              pending =
                pendingAttestations.filterIt(not (it.finished())).mapIt(
                  it.cancelAndWait()
                )
      ...
      statistics =
        block:
          ...
          try:
            ...
            let
              pending =
                pendingAggregates.filterIt(not (it.finished())).mapIt(
                  it.cancelAndWait()
                )
            await noCancel allFutures(pending)
            raise exc
