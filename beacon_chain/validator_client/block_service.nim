...
proc prepareRandao(
  ...
  let
    ...
    # duty slot.
    currentSlot =
      (await vc.checkedWaitForSlot(destSlot, destOffset, false)).valueOr:
...
proc publishBlock(
        ...
        let
          blindedBlockFut =
            if vc.config.payloadBuilderEnable:
              vc.produceBlindedBlock(
                currentSlot, slot, randaoReveal, graffiti, validator
              )
            else:
              nil
        let
  ...
  if beaconBlocks.blindedBlock.isSome():
    let
      ...
      notSlashable =
        vc.attachedValidators[].slashingProtection.registerBlock(
    ...
    if notSlashable.isOk():
      ...
      let
        signedBlock =
          ForkedSignedBlindedBeaconBlock.init(
...
proc proposeBlock(
  ...
  let
    currentSlot =
      (await vc.checkedWaitForSlot(slot, ZeroTimeDiff, false)).valueOr:
        ...
        block:
          ...
          try:
            ...
            if resp.status == 200:
              ...
              debug "Unable to to obtain events stream",
                code = resp.status, reason = reason
