...
proc pollForSyncCommitteeDuties*(service: DutiesServiceRef) {.async.} =
  ...
  let
    ...
    currentEpoch = currentSlot.epoch()
    altairEpoch =
      vc.runtimeConfig.altairEpoch.valueOr:
        return
  ...
  if vc.attachedValidators[].count() != 0:
    ...
    block:
      let
        moment = Moment.now()
        sigres =
          await vc.fillSyncCommitteeSelectionProofs(
            currentSlot, currentSlot + Epoch(AGGREGATION_PRE_COMPUTE_EPOCHS)
          )
    ...
    let
      periods =
        block:
          ...
          let
            lookaheadSlot =
              currentSlot + SUBSCRIPTION_LOOKAHEAD_EPOCHS * SLOTS_PER_EPOCH
