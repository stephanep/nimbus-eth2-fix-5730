...
proc get*[T](s: DbSeq[T], idx: int64): T =
  ...
  let
    queryRes =
      s.selectStmt.exec(idx + 1) do(recordBytes: openArray[byte]):
        try:
...
proc new*(T: type BeaconChainDBV0, db: SqStoreRef, readOnly = false): BeaconChainDBV0 =
  var
    ...
    stateStoreV0 =
      kvStore db.openKvStore("state_no_validators", readOnly = db.readOnly or readOnly).expectDb()
...
proc new*(
  ...
  block:
    var
      immutableValidatorsDb1 =
        DbSeq[ImmutableValidatorData].init(db, "immutable_validators", readOnly = true).expectDb()
...
proc getSnappySSZ[T](db: KvStoreRef, key: openArray[byte], output: var T): GetResult =
  ...
  proc decode(data: openArray[byte]) =
    status =
      if decodeSnappySSZ(data, outputPtr[]):
        GetResult.found
      else:
        GetResult.corrupted
