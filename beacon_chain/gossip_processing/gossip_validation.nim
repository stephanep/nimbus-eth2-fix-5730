  ...
  let
    sig =
      if checkSignature:
        ...
        let
          deferredCrypto =
            batchCrypto.scheduleAttestationCheck(
  ...
  let
    sig =
      if checkSignature:
        let
          deferredCrypto =
            batchCrypto.scheduleAggregateChecks(
...
proc validateBlsToExecutionChange*(
  ...
  withState(pool.dag.headState):
    when consensusFork < ConsensusFork.Capella:
      ...
      let
        deferredCrypto =
          batchCrypto.scheduleBlsToExecutionChangeCheck(
  ...
  let
    sig =
      if checkSignature:
        ...
        let
          deferredCrypto =
            batchCrypto.scheduleSyncCommitteeMessageCheck(
  ...
  let
    sig =
      if checkSignature:
        let
          deferredCrypto =
            batchCrypto.scheduleContributionChecks(
