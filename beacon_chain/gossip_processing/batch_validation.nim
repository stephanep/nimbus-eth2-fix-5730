...
proc processLoop(batchCrypto: ref BatchCrypto) {.async.} =
  ...
  while batchCrypto[].batches.len() > 0:
    ...
    batchCrypto[].verifiers[verifier].inflight =
...
proc scheduleAggregateChecks*(
  ...
  let
    aggregatorFut =
      batchCrypto.verifySoon("scheduleAggregateChecks.aggregator"):
    ...
    slotFut =
      batchCrypto.verifySoon("scheduleAggregateChecks.selection_proof"):
        slot_signature_set(
          fork,
          batchCrypto[].genesis_validators_root,
          aggregate.data.slot,
    ...
    aggregateFut =
      batchCrypto.verifySoon("scheduleAggregateChecks.aggregate"):
        attestation_signature_set(
          fork,
          batchCrypto[].genesis_validators_root,
          aggregate.data,
...
proc scheduleSyncCommitteeMessageCheck*(
  ...
  let
    ...
    fut =
      batchCrypto.verifySoon("scheduleSyncCommitteeMessageCheck"):
...
proc scheduleContributionChecks*(
  ...
  let
    aggregatorFut =
      batchCrypto.verifySoon("scheduleContributionAndProofChecks.aggregator"):
    ...
    proofFut =
      batchCrypto.verifySoon("scheduleContributionAndProofChecks.selection_proof"):
    ...
    contributionFut =
      batchCrypto.verifySoon("scheduleContributionAndProofChecks.contribution"):
        sync_committee_message_signature_set(
          fork,
          batchCrypto[].genesis_validators_root,
          contribution.slot,
  ...
  let
    ...
    sig =
      signedBLSToExecutionChange.signature.load().valueOr:
        return err(
            "scheduleBlsToExecutionChangeCheck: invalid validator change signature"
          )
    fut =
      batchCrypto.verifySoon("scheduleContributionAndProofChecks.contribution"):
