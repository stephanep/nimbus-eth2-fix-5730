      ...
      blobs =
        when typeof(signedBlock).kind >= ConsensusFork.Deneb:
          if self.blobQuarantine[].hasBlobs(signedBlock):
            ...
            if not self.quarantine[].addBlobless(
...
proc processBlsToExecutionChange*(
  ...
  let
    v =
      await self.validatorChangePool[].validateBlsToExecutionChange(
        self.batchCrypto,
...
proc processSyncCommitteeMessage*(
  ...
  let
    v =
      await validateSyncCommitteeMessage(
        self.dag,
        self.quarantine,
        self.batchCrypto,
...
proc processSignedContributionAndProof*(
  ...
  # Potential under/overflows are fine; would just create odd metrics and logs
  let
    delay = wallTime - contributionAndProof.message.contribution.slot.start_beacon_time
