...
proc loadKeystores*(node: SigningNodeRef) =
  ...
  for keystore in listLoadableKeystores(node.config, node.keystoreCache):
    ...
    of KeystoreKind.Local:
      discard node.attachedValidators.addValidator(
          keystore, feeRecipient, defaultGasLimit
        )
          ...
          let
            ...
            signature =
              get_block_signature(
                forkInfo.fork,
                forkInfo.genesis_validators_root,
                request.beaconBlockHeader.data.slot,
                blockRoot,
                validator.data.privateKey,
              ).toValidatorSig().toHex()
