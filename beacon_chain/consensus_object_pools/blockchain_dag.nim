  ...
  let
    summary =
      dag.db.getBeaconBlockSummary(bid.root).valueOr:
        return false
    parentMinSlot =
      dag.db.getBeaconBlockSummary(summary.parent_root).map(
        proc(x: auto): auto =
            x.slot
      ).valueOr:
...
proc updateBeaconMetrics(
  ...
  withState(state):
    ...
    let
      active_validators =
        count_active_validators(forkyState.data, forkyState.data.slot.epoch, cache).toGaugeValue
...
proc init*(
  ...
  for blck in db.getAncestorSummaries(head.root):
    ...
    if not foundHeadState:
      foundHeadState =
        db.getState(cfg, blck.root, blck.summary.slot..slot, dag.headState, noRollback)
  ...
  dag.lastHistoryPruneBlockHorizon =
    block:
  ...
  dag.db.withManyWrites:
    if stateHorizon > dag.tail.slot:
      ...
      while cur.isSome():
        ...
        if dag.db.containsState(dag.cfg, bs.bid.root, bs.slot..bs.slot, legacy = first):
          if first:
...
proc getProposalState*(
  ...
  if getStateField(state[], slot) < slot:
    process_slots(dag.cfg, state[], slot, cache, info, {skipLastStateRootCalculation}).expect(
