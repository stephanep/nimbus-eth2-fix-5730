...
func shortProtocolId(protocolId: string): string =
  let
    start =
      if protocolId.startsWith(requestPrefix):
        requestPrefix.len
      else:
        0
    ends =
  ...
  while true:
    ...
    if wantedAttnetsCount > 0 or wantedSyncnetsCount > 0 or
      ...
      let
        ...
        discoveredNodes =
          await node.discovery.queryRandom(
...
proc p2pProtocolBackendImpl*(p: P2PProtocol): Backend =
  ...
  result.implementMsg =
    proc(msg: p2p_protocol_dsl.Message) =
        if msg.kind == msgResponse:
          return
        ...
        if msg.userHandler != nil:
          ...
          if OutputParamType == nil:
            ...
            if msg.kind == msgRequest:
              userHandlerCall =
                newCall(
                  ident"sendUserHandlerResultAsChunkImpl", streamVar, userHandlerCall
                )
          else:
            ...
            msg.response.recName =
              if isChunkStream:
                newTree(
                  nnkBracketExpr, ident"List", OutputParamType[1], OutputParamType[2]
                )
              else:
                OutputParamType[1]
  ...
  result.implementProtocolInit =
    proc(p: P2PProtocol): NimNode =
...
proc newBeaconSwitch(
  ...
  sb.withPrivateKey(seckey).withAddress(address).withRng(rng).withNoise().withMplex(
    chronos.minutes(5), chronos.minutes(5)
  ).withMaxConnections(config.maxPeers).withAgentVersion(config.agentString).withTcpTransport(
...
proc broadcastSignedContributionAndProof*(
  ...
  let
    topic =
      getSyncCommitteeContributionAndProofTopic(
...
proc broadcastLightClientFinalityUpdate*(
  ...
  let
    topic = getLightClientFinalityUpdateTopic(node.forkDigestAtEpoch(msg.contextEpoch))
  node.broadcast(topic, msg)
