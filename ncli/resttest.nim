...
proc openConnection*(
  ...
  if uri.scheme == "http":
    ...
    let
      tlsstream = newTLSClientAsyncStream(treader, twriter, uri.hostname, flags = flags)
    return HttpConnectionRef(
...
proc prepareRequest(
  ...
  let
    (requestBodyType, requestBodyData) =
      block:
                ...
                if len(jstart.elems) != 0:
                  for elem in jstart.elems:
                    ...
                    of JString:
                      ...
                      return err(
                          "`response.body` element has incorrect `start`" & " option"
                        )
              else:
  ...
  let
    rlen =
      try:
        ...
        let
          res =
            await conn.reader.readUntil(
              addr headersBuf[0], len(headersBuf), HeadersMark
            )
      ...
      let
        flags =
          block:
            ...
            res
      return TestResult.failure(
          TestResultKind.ValidationError, times = times, flags = flags
        )
