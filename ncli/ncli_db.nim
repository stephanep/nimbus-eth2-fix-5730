...
proc cmdPutState(conf: DbConf, cfg: RuntimeConfig) =
  ...
  for file in conf.stateFile:
    if shouldShutDown:
      quit QuitSuccess
    let
      state = newClone(readSszForkedHashedBeaconState(cfg, readAllBytes(file).tryGet()))
    withState(state[]):
...
proc cmdValidatorPerf(conf: DbConf, cfg: RuntimeConfig) =
  ...
  for bi in 0..<blockRefs.len:
    blck =
      db.getBlock(
        blockRefs[blockRefs.len - bi - 1].root, phase0.TrustedSignedBeaconBlock
      ).get()
  ...
  while getStateField(state[], slot) < ends:
    process_slots(dag.cfg, state[], getStateField(state[], slot) + 1, cache, info, {}).expect(
      "Slot processing can't fail with correct inputs"
    )
...
proc cmdValidatorDb(conf: DbConf, cfg: RuntimeConfig) =
  ...
  for bi in 0..<blockRefs.len:
    ...
    withBlck(forkedBlock):
      ...
      let
        res = state_transition_block(cfg, tmpState[], forkyBlck, cache, {}, noRollback)
      if res.isErr:
