  ...
  try:
    while not (stream.atEnd()):
      ...
      if m.msg == "Attestation sent":
        ...
        let
          rm = Json.decode(line, AttestationReceivedMessage, allowUnknownFields = true)
        ...
        let
          sm =
            Json.decode(
              line, AggregatedAttestationSentMessage, allowUnknownFields = true
            )
  ...
  try:
    while not (stream.atEnd()):
      ...
      if m.msg == "Sync committee message sent":
        ...
        let
          rm = Json.decode(line, ContributionReceivedMessage, allowUnknownFields = true)
    ...
    if ("msg" in node) and ("message_id" in node) and ("ts" in node):
      ...
      if message == "Incoming pubsub message received":
        let
          msg =
            GossipMessage.init(
              Incoming, node["message_id"].getStr(), node["ts"].getStr()
            )
        result.add(msg)
        ...
        let
          msg =
            GossipMessage.init(
              Outgoing, node["message_id"].getStr(), node["ts"].getStr()
            )
        result.add(msg)
