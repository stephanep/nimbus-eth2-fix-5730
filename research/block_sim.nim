...
proc makeSimulationBlock(
  ...
  let
    res = process_block(cfg, state.data, blck.asSigVerified(), verificationFlags, cache)
...
cli do(
  ...
  proc getNewBlock[T](
    ...
    let
      ...
      eth1ProposalData =
        eth1Chain.getBlockProposalData(
  ...
  proc proposeCapellaBlock(slot: Slot) =
    ...
    dag.withUpdatedState(tmpState[], dag.getBlockIdAtSlot(slot).expect("block")):
      let
        ...
        added =
          dag.addHeadBlock(verifier, newBlock) do(
            blckRef: BlockRef,
