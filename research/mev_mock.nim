...
proc getPrevRandao(restClient: RestClientRef): Future[Opt[Eth2Digest]] {.async.} =
  let
    resp: RestResponse[rest_types.GetStateRandaoResponse] =
...
proc getWithdrawals(restClient: RestClientRef): Future[Opt[seq[Withdrawal]]] {.async.} =
  let
    resp: RestResponse[rest_types.GetNextWithdrawalsResponse] =
...
proc getInfo(parent_hash: Eth2Digest): Future[Opt[capella.ExecutionPayload]] {.async.} =
  ...
  let
    prev_randao =
      (await getPrevRandao(restClient)).valueOr:
        return Opt.none capella.ExecutionPayload
    parent_block_info =
      (await getParentBlock(restClient)).valueOr:
        return Opt.none capella.ExecutionPayload
    withdrawals =
      (await getWithdrawals(restClient)).valueOr:
        return Opt.none capella.ExecutionPayload
...
proc setupEngineAPI*(
  ...
  router.api(MethodGet, "/eth/v1/builder/header/{slot}/{parent_hash}/{pubkey}") do(
    ...
    let
      execution_payload =
        (await getInfo(parent_hash.get)).valueOr:
          return RestApiResponse.jsonError(
              Http400, "Error getting parent head information"
            )
